	...
	var $preload = $.preload = function( original, settings ){
		...
		var sources = $.map( original, function( source ){
			...
			if( typeof settings.placeholder == 'string' && source.src ) { // Placeholder Mode, if it's an image, set it.
				internal++;
				x = (($(source).attr("width"))/2)-8;
				y = (($(source).attr("height"))/2)-8;
				html = '<div id="load'+internal+'" style="position:absolute; padding:'+y+'px 0 0 '+x+'px;"><img src="'+loader.src+'" /></div>';
				$(source).parent().parent().before(html);
		...
		function handler( e ){
			...
			$(orig).parent().fadeOut(0);
			$(orig).parent().css('visibility','visible');
			...
			if( settings.onComplete )
				...
				$(orig).parent().fadeIn(300);
				$("#load"+(data.index+1)).fadeOut(300, function() {
					$(this).remove();
				});
